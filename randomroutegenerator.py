	...
	with open("intersection3.rou.xml", "w") as routes:
		...
		for i in range(N):		
				...
				if i%8==3:		
					...
					vehNr += 1
				if i%8==4:		
					print('    <vehicle id="right_%i" type="genericvehicle" route="right" depart="%i" />' % (vehNr, i), file=routes)
					vehNr += 1
				if i%8==5:		
					print('    <vehicle id="left_%i" type="genericvehicle" route="left" depart="%i" />' % (vehNr, i), file=routes)
					vehNr += 1
				if i%8==6:		
					print('    <vehicle id="left_%i" type="genericvehicle" route="left" depart="%i" />' % (vehNr, i), file=routes)
					vehNr += 1
